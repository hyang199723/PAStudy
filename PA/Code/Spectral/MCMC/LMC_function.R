                 ...
                 sd_rho=10, iters=3000, burn=1000, thin=1, update=10)
{
  ...
  out=list(keep.rangeU,keep.rangeV,keep.sigmaU,keep.sigmaV,keep.taue1,keep.taue2,keep.A,keep.Y1.M,
           keep.Y2.M,keep.u1,keep.u2,keep.v2,aru,arv)
  names(out)=c('rangeU','rangeV','sigmaU','sigmaV','tau1','tau2','A','Y1.m','Y2.m','U1','U2','V2','aru','arv')
  return(out)
}
...
#       keep.A[, iter] = A
#       
#       keep.u1[,,iter]=as.matrix(U1)
#       keep.u2[,,iter]=as.matrix(U2)
#       keep.v2[,,iter]=as.matrix(V2)
#       
