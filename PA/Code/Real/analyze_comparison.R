...
load("~/Documents/SofiRuiz/PAdata/all_reail_init.RData")
source('LMC_Real.R') # last version here
...
iters = 3000
burn=1000
# to save Means and Vars Uv and cor
Mu1=list(); Mu2=list(); Mv2=list()
Vu1=list(); Vu2=list(); Vv2=list()
Mcor=list();Vcor=list()
...
Mcor[[k]]=fit$MeanCorrY1Y2
Vcor[[k]]=fit$VarCorrY1Y2
...
ggplot(df1%>% filter(model=='lmc'))+
  geom_boxplot(aes(y=values,x=index))+facet_wrap(~index,scale='free')+
  theme_bw()+ ggtitle('Results LMC' )
...
# Plot Mean 
ggplot(df1%>% filter(model=='mean'))+
  geom_boxplot(aes(y=values,x=index))+facet_wrap(~index,scale='free')+
  theme_bw()+ ggtitle('Results Mean' )


df1 %>% filter(model=='mean', index=='elapsed')
...
ggplot(df1%>% filter(model=='lmc'))+
  geom_boxplot(aes(y=values,x=index))+facet_wrap(~index,scales="free")+
  theme_bw()
