...
pa$Timestamp <- force_tz(pa$Timestamp, 'UTC')
pa$Timestamp <- with_tz(pa$Timestamp, 'EST')

start <- as.POSIXct('2020-01-01 00:00:00', tz = 'EST')
end <- as.POSIXct('2020-12-31 23:00:00', tz = 'EST')
pa <- pa[pa$Timestamp >= start & pa$Timestamp <= end, ]

write.csv(epa, 'FRM_2020_Hourly_Formatted.csv', row.names = FALSE)
write.csv(pa, 'PA_2020_Hourly_Formatted.csv', row.names = FALSE)
